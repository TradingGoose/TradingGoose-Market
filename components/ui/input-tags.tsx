...
export const InputTags = forwardRef<HTMLInputElement, InputTagsProps>(({ value, onChange, ...props }, ref) => {
  ...
  return (
    <>
      ...
      <div className='min-h-[2.5rem] flex flex-wrap items-center gap-2 overflow-y-auto rounded-md border p-2'>
        {value.map(item => (
        ...
        {!value.length && <span className='text-xs text-muted-foreground'>No exchange codes.</span>}
      </div>
    </>
  )
})
