...
export function TimeZonesTable({ data, totalCount }: TimeZonesTableProps = {}) {
  ...
  const [editingTimeZone, setEditingTimeZone] = useState<TimeZoneRow | null>(null)
  ...
  const handleEditTimeZone = useCallback((timeZone: TimeZoneRow) => {
    setEditingTimeZone(timeZone)
    setIsEditorOpen(true)
  }, [])
  const handleDeleteTimeZone = useCallback(
    async (timeZone: TimeZoneRow) => {
      const label = timeZone.name || timeZone.id
      if (!window.confirm(`Delete time zone ${label}? This cannot be undone.`)) {
        return
      }
      try {
        const response = await fetch(`/api/time-zones/${timeZone.id}`, { method: 'DELETE' })
        if (!response.ok) {
        ...
        setTableData(prev => prev.filter(row => row.id !== timeZone.id))
        ...
        if (editingTimeZone?.id === timeZone.id) {
          setEditingTimeZone(null)
          setIsEditorOpen(false)
        }
      } catch (error) {
        const message =
          error instanceof Error ? error.message : 'Unable to delete time zone.'
        console.error('[time-zones] delete failed:', error)
        window.alert(message)
      }
    },
    [editingTimeZone?.id, isRemote, pagination.pageIndex, tableData.length]
  )

  const handleEditorOpenChange = (open: boolean) => {
    setIsEditorOpen(open)
    if (!open) setEditingTimeZone(null)
  }

  const handleTimeZoneUpdated = (updated: TimeZoneRow) => {
    setTableData(prev => prev.map(row => (row.id === updated.id ? updated : row)))
    setEditingTimeZone(updated)
  }

  const handleTimeZoneCreated = (created: TimeZoneRow) => {
    setTableData(prev => {
  ...
  useEffect(() => {
    ...
    fetch(`/api/time-zones?${params.toString()}`, { signal: controller.signal })
      .then(async response => {
        ...
        const payload = (await response.json()) as TimeZonesApiResponse
        setTableData(payload.data)
        setRemoteTotal(payload.total)
      })
      .catch(error => {
        if (error instanceof DOMException && error.name === 'AbortError') return
        setLoadError('Unable to load time zones data.')
        setTableData([])
        setRemoteTotal(0)
      })
      .finally(() => {
  ...
  const columns = useMemo(
    () => buildTimeZoneColumns(handleEditTimeZone, handleDeleteTimeZone),
    [handleEditTimeZone, handleDeleteTimeZone]
  )
  ...
  const exportToJSON = async () => {
    try {
      const response = await fetch('/api/time-zones/export')
      if (!response.ok) {
        throw new Error(`Request failed with ${response.status}`)
      }
      ...
      const fallbackName = `time-zones-export-${new Date().toISOString().split('T')[0]}.json`
      ...
      console.error('[time-zones] export failed:', error)
    }
  }
