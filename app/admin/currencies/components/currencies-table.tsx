...
export function CurrenciesTable({ data, totalCount }: CurrenciesTableProps = {}) {
  ...
  const handleEditCurrency = useCallback((currency: CurrencyRow) => {
    setEditingCurrency(currency)
    setIsEditorOpen(true)
  }, [])
  const handleDeleteCurrency = useCallback(
    async (currency: CurrencyRow) => {
      const label = currency.code || currency.id
      if (!window.confirm(`Delete currency ${label}? This cannot be undone.`)) {
        return
      }
      try {
        const response = await fetch(`/api/currencies/${currency.id}`, { method: 'DELETE' })
        if (!response.ok) {
        ...
        setTableData(prev => prev.filter(row => row.id !== currency.id))
        ...
        if (editingCurrency?.id === currency.id) {
        ...
        console.error('[currencies] delete failed:', error)
        window.alert(message)
      }
    },
    [editingCurrency?.id, isRemote, pagination.pageIndex, tableData.length]
  )
  ...
  const handleCurrencyUpdated = (updated: CurrencyRow) => {
    setTableData(prev => prev.map(row => (row.id === updated.id ? updated : row)))
    setEditingCurrency(updated)
  }

  const handleCurrencyCreated = (created: CurrencyRow) => {
    setTableData(prev => {
  ...
  useEffect(() => {
    ...
    fetch(`/api/currencies?${params.toString()}`, { signal: controller.signal })
      .then(async response => {
        ...
        const payload = (await response.json()) as CurrenciesApiResponse
        setTableData(payload.data)
        setRemoteTotal(payload.total)
      })
      .catch(error => {
        if (error instanceof DOMException && error.name === 'AbortError') return
        setLoadError('Unable to load currencies data.')
        setTableData([])
        setRemoteTotal(0)
      })
      .finally(() => {
  ...
  const columns = useMemo(
    () => buildCurrencyColumns(handleEditCurrency, handleDeleteCurrency),
    [handleEditCurrency, handleDeleteCurrency]
  )
  ...
  const exportToJSON = async () => {
    try {
      const response = await fetch('/api/currencies/export')
      if (!response.ok) {
        throw new Error(`Request failed with ${response.status}`)
      }
      ...
      const fallbackName = `currencies-export-${new Date().toISOString().split('T')[0]}.json`
      ...
      console.error('[currencies] export failed:', error)
    }
  }
