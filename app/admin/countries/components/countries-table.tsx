...
import { CountryEditDialog } from './countries-edit-dialog'
...
export function CountriesTable({ data, totalCount }: CountriesTableProps = {}) {
  ...
  const handleEditCountry = useCallback((country: CountryRow) => {
    setEditingCountry(country)
    setIsEditorOpen(true)
  }, [])
  const handleDeleteCountry = useCallback(
    async (country: CountryRow) => {
      const label = country.code || country.id
      if (!window.confirm(`Delete country ${label}? This cannot be undone.`)) {
        return
      }
      try {
        ...
        setTableData(prev => prev.filter(row => row.id !== country.id))
        ...
        console.error('[countries] delete failed:', error)
        window.alert(message)
      }
    },
    [editingCountry?.id, isRemote, pagination.pageIndex, tableData.length]
  )
  ...
  const handleCountryUpdated = (updated: CountryRow) => {
    setTableData(prev => prev.map(row => (row.id === updated.id ? updated : row)))
    setEditingCountry(updated)
  }

  const handleCountryCreated = (created: CountryRow) => {
    setTableData(prev => {
  ...
  useEffect(() => {
    ...
    fetch(`/api/countries?${params.toString()}`, { signal: controller.signal })
      .then(async response => {
        ...
        const payload = (await response.json()) as CountriesApiResponse
        setTableData(payload.data)
        setRemoteTotal(payload.total)
      })
      .catch(error => {
        if (error instanceof DOMException && error.name === 'AbortError') return
        setLoadError('Unable to load countries data.')
        setTableData([])
        setRemoteTotal(0)
      })
      .finally(() => {
  ...
  const columns = useMemo(
    () => buildCountryColumns(handleEditCountry, handleDeleteCountry),
    [handleEditCountry, handleDeleteCountry]
  )
  ...
  const exportToJSON = async () => {
    try {
      const response = await fetch('/api/countries/export')
      if (!response.ok) {
        throw new Error(`Request failed with ${response.status}`)
      }
      ...
      const fallbackName = `countries-export-${new Date().toISOString().split('T')[0]}.json`
      ...
      <div className='flex min-h-0 w-full flex-1 flex-col gap-4 overflow-hidden min-w-0'>
        <div className='flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden rounded-xl border bg-card'>
          <div className='flex flex-col gap-4 border-b p-6 sm:flex-row sm:flex-wrap sm:items-end sm:justify-between'>
            <TableFilter column={table.getColumn('id')!} placeholder='Search country (ID, code, or name)' hideLabel />
