...
import { fetchCountriesForExport } from "../lib";
import { createSystemAdminRoutePolicy } from "@/lib/market-api/core/entity-route";


export const runtime = "nodejs";
const routePolicy = createSystemAdminRoutePolicy("/api/countries/export");
...
export async function GET(_request: Request) {
  ...
  try {
    ...
    const data = await fetchCountriesForExport();
    const body = JSON.stringify(data, null, 2);
    const filename = `countries-export-${new Date().toISOString().split("T")[0]}.json`;
    ...
    console.error("[countries:export] API error:", error);
